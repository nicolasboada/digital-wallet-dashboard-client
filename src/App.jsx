...
const fetchExchangeRates = async () => {
  const response = await fetch('https://min-api.cryptocompare.com/data/price?fsym=ETH&tsyms=USD,EUR')
    ...
    if (userId.length > 0) {
      ...
      const updatedWallets = await Promise.all(
        updatedWalletsData.map(async (wallet) => {
          ...
          const res = await axios.put(`${BASE_URL}/wallets/${id}`, {...updatedwallet,_id: id})
          return res.data
          })
      )
...
const fetchWalletData = async (address) => {
  try {
    ...
    const result = await Promise.allSettled(
      [
        fetch(`https://api.etherscan.io/api?module=account&action=balance&address=${address}&tag=latest&apikey=${ApiKeyToken}`).then(res => res.json()),
        fetch(`https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=1&sort=asc&apikey=${ApiKeyToken}`).then(res => res.json()),
      ]
      )
